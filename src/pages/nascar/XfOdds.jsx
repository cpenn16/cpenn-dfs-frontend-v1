// src/pages/nascar/XfinityBetting.jsx
...
function BettingTableBlock({ title, source, compact = false, maxHeight = "62vh", bounded = true }) {
        ...
        <div className="rounded-xl border bg-white shadow-sm overflow-auto" style={{ maxHeight }}>
          <table className="w-full table-fixed border-separate" style={{ borderSpacing: 0 }}>
            ...
            <tbody>
              ...
              {!sortedRows.length && (
                <tr>
                  <td className="px-2 py-1 text-xs text-gray-500" colSpan={(visibleColNames.length ? visibleColNames : allCols).length}>
                    No rows.
                  </td>
                </tr>
              )}
...
function FinishDistTable({ source, compact = false, maxHeight = "62vh" }) {
  const { data, err, loading } = useJson(source);
...
export default function CupBetting() {
  ...
  const MAIN = `${BASE}data/nascar/xfinity/latest/odds_main.json`;
  const EXTRA = `${BASE}data/nascar/xfinity/latest/odds_extra.json`;
  const H2H  = `${BASE}data/nascar/xfinity/latest/h2h_matrix.json`;
  const FINISH = `${BASE}data/nascar/xfinity/latest/finish_dist.json`;
    ...
    <div className="px-5 py-6">
      ...
      <div className="max-w-[1200px] mx-auto">
        <div className="flex flex-col sm:flex-row sm:items-end gap-3 mb-4">
          <h1 className="text-2xl sm:text-3xl font-extrabold">NASCAR Xfinity — Betting Sims</h1>
