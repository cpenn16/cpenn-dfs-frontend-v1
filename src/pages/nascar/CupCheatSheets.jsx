...
export default function CupCheatSheets() {
  ...
  const SOURCE = `${BASE}data/nascar/cup/latest/cheatsheets.json`;
  ...
  return (
    <div className="px-5 py-6">
      <h1 className="text-2xl sm:text-3xl font-extrabold mb-3">NASCAR Cup — Cheat Sheets</h1>
      ...
      {!loading && !err && (!tables || tables.length === 0) && (
        <div className="text-sm text-gray-600">
          No tables found. Ensure <code>{`${BASE}data/nascar/cup/latest/cheatsheets.json`}</code> exists.
        </div>
      )}
