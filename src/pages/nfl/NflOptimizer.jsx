...
// Persist a Set<> in localStorage as an array, but expose a Set API to the component
...
export default function NFLOptimizer() {
  ...
  const [numLineups, setNumLineups] = useStickyState("nflOpt.N", 10);
  ...
  // Persist these per-site so refresh doesn't clear them
  ...
  // live clock so build chips' "time ago" updates
  ...
  // On site switch, clear live results, retain persisted constraints by site
  useEffect(() => {
    setLineups([]);
    setStopInfo(null);
    setProgressActual(0);
  ...
  return (
    <div className="px-4 md:px-6 py-5">
          ...
          <tbody>
            ...
            {!loading &&
              ...
              displayRows.map((r) => (
                ...
                >
                  <td className={cell}>
                    <input type="checkbox" checked={locks.has(r.name)} onChange={() => toggleLock(r.name)} />
                  </td>
                  <td className={cell}>
                    <input type="checkbox" checked={excls.has(r.name)} onChange={() => toggleExcl(r.name)} />
                  </td>
                  ...
                  <td className={`${cell} tabular-nums`}>
                    <div className="flex flex-col items-center leading-tight">
                      <div>{fmt0(r.salary)}</div>
                      <div className="text-[10px] text-gray-500">{fmt1(r.pown * 100)}%</div>
                    </div>
                  </td>
                  ...
                  <td className={cell}>
                    <div className="inline-flex items-center gap-1">
                      <button
                        className="px-1.5 py-0.5 border rounded"
                        onClick={() =>
                          setMinPct((m) => ({ ...m, [r.name]: clamp((num(m[r.name]) || 0) - 5, 0, 100) }))}
                        title="-5%"
                      >
                        –
                      </button>
                      <input
                        ...
                        onChange={(e) => setMinPct((m) => ({ ...m, [r.name]: e.target.value }))}
                        placeholder="—"
                      />
                      <button
                        className="px-1.5 py-0.5 border rounded"
                  ...
                  <td className={cell}>
                    <div className="inline-flex items-center gap-1">
                      <button
                        className="px-1.5 py-0.5 border rounded"
                        onClick={() =>
                          setMaxPct((m) => ({ ...m, [r.name]: clamp((num(m[r.name]) || 100) - 5, 0, 100) }))}
                        title="-5%"
                      >
                        –
                      </button>
                      <input
                        ...
                        onChange={(e) => setMaxPct((m) => ({ ...m, [r.name]: e.target.value }))}
                        placeholder="—"
                      />
                      <button
                        className="px-1.5 py-0.5 border rounded"
      ...
      <div className="mb-3">
        ...
        {builds.length === 0 ? (
          <div className="text-xs text-gray-500">No saved builds yet.</div>
        ) : (
          <div className="flex flex-wrap gap-2">
            {builds
              .slice()
              ...
              .map((b) => {
                ...
                return (
                  <div
                    key={b.id}
                    className={`group inline-flex items-center gap-2 ${cls.btn.chip} ${
                      isActive ? "border-blue-600 bg-blue-50 text-blue-800" : "bg-white border-gray-300"
                    }`}
                  >
                    <button
                      className="px-0.5 py-1"
                      onClick={() => selectBuild(b.id)}
                      title={`${b.lineups?.length || 0} lineups`}
                    >
                      <span className="font-semibold">{b.name}</span>
                      <span className="ml-1 text-[10px] opacity-70">
                        {timeAgo(b.ts)} • {b.lineups?.length || 0}
                      </span>
                    </button>
                    <button
                      className={`${cls.btn.iconSm} ml-1`}
                      title="Rename"
                      onClick={() => {
...
function PlayerMultiPicker({ allPlayers, value, onChange, placeholder = "Add players…" }) {
  ...
  const suggestions = React.useMemo(() => {
    const needle = q.trim().toLowerCase();
    if (!needle) return [];
    return allPlayers.filter((n) => !selected.has(n) && n.toLowerCase().includes(needle)).slice(0, 12);
  ...
  return (
    <div>
      ...
      <div className={`overflow-auto ${maxHeightClass}`}>
        <table
          className="min-w-full text-[12px] border-separate"
