    ...
    const cells = ordered.slice(0, slotsCount).map((meta) => {
      ...
      if (!rec && pos === "DST") {
        ...
        if (!rec && metaTeam) {
          ...
          for (const k of tryKeys) { rec = rec || idIndex.get(k); if (rec) break; }
        }
        if (!rec) {
          const inferred = inferTeamFromNameForDST(name);
          if (inferred) {
            rec = idIndex.get(keyTeam(inferred, "DST")) ||
...
export default function NFLOptimizer() {
  ...
  const displayRows = useMemo(() => {
    const posOK = (pos) => (posFilter === "ALL" ? true : posFilter === "FLEX" ? ["RB","WR","TE"].includes(pos) : pos === posFilter);
  ...
  const sortable = new Set(["pos","team","opp","salary","time","proj","val","floor","ceil","pown","opt","usage"]);
  const setSort = (col) => {
...
function PlayerMultiPicker({ allPlayers, value, onChange, placeholder = "Add players…" }) {
  ...
  const add = (name) => { if (!selected.has(name)) onChange([...value, name]); setQ(""); };
  ...
  return (
    <div className="w-full">
      <div className="flex flex-wrap items-center gap-2 border rounded-md px-2 py-1.5">
        {value.map((name) => (
          <span key={name} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-blue-50 text-blue-800 border border-blue-200 text-[12px]">
            {name}
            <button onClick={() => remove(name)} className="leading-none text-blue-600 hover:text-blue-900" title="Remove">×</button>
          </span>
        ))}
        <input
          value={q}
          onChange={(e) => setQ(e.target.value)}
          onKeyDown={(e) => { if (e.key === "Enter" && suggestions.length) { e.preventDefault(); add(suggestions[0]); } }}
      ...
      {!!q && suggestions.length > 0 && (
        <div className="mt-1 max-h-48 overflow-auto bg-white border rounded-md shadow-sm">
          {suggestions.map((n) => (
            <button key={n} className="block w-full text-left px-3 py-1.5 hover:bg-blue-50 text-sm" onClick={() => add(n)}>{n}</button>
          ))}
        </div>
      )}
    </div>
        ...
        <div className="space-y-1 max-h-48 overflow-auto">
          {sel.map((t) => (
            <div key={t} className="flex items-center justify-between gap-2">
              <span className="inline-flex items-center gap-1"><TeamPill abbr={t} /></span>
              <input
