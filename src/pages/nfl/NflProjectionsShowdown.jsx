...
export default function NflProjectionsShowdown() {
  ...
  const rows = useMemo(() => {
    return raw.map((r) => {
      ...
      // projections (with base DK/FD available again via config)
      const dk_proj = pick(r, ["DK Proj", "dk_proj", "dk_projection", "DK Flex Proj", "dk_flex_proj"]);
      const fd_proj = pick(r, ["FD Proj", "fd_proj", "fd_projection", "FD Flex Proj", "fd_flex_proj"]);
  ...
  const filtered = useMemo(() => {
    ...
    if (!s) return rows;
    return rows.filter((r) => `${r.player} ${r.pos} ${r.team} ${r.opp}`.toLowerCase().includes(s));
  }, [rows, q]);
      ...
      <div className="flex items-center justify-between gap-3 mb-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="inline-flex items-center gap-1 rounded-xl bg-gray-100 p-1">
            {["dk", "fd"].map((k) => (
              <button
                key={k}
                onClick={() => setSite(k)}
                className={`px-3 py-1.5 rounded-lg text-sm inline-flex items-center gap-1 ${site === k ? "bg-white shadow font-semibold" : "text-gray-700"}`}
                title="Toggle site"
              >
          ...
          <div className="inline-flex items-center gap-1 rounded-xl bg-gray-100 p-1">
            {site === "dk" ? (
              <>
                <button className={`px-3 py-1.5 rounded-lg text-sm ${slot === "flex" ? "bg-white shadow font-semibold" : "text-gray-700"}`} onClick={() => setSlot("flex")}>DK Flex</button>
                <button className={`px-3 py-1.5 rounded-lg text-sm ${slot === "cpt" ? "bg-white shadow font-semibold" : "text-gray-700"}`} onClick={() => setSlot("cpt")}>DK CPT</button>
              </>
            ) : (
              <>
                <button className={`px-3 py-1.5 rounded-lg text-sm ${slot === "flex" ? "bg-white shadow font-semibold" : "text-gray-700"}`} onClick={() => setSlot("flex")}>FD Flex</button>
                <button className={`px-3 py-1.5 rounded-lg text-sm ${slot === "mvp" ? "bg-white shadow font-semibold" : "text-gray-700"}`} onClick={() => setSlot("mvp")}>FD MVP</button>
              </>
            )}
          </div>

          {/* search */}
          <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search player / team / opp…" className="px-3 py-2 rounded-lg border w-64" />

          {/* export */}
          <button className="ml-1 px-3 py-2 rounded-lg bg-blue-600 text-white text-sm hover:bg-blue-700" onClick={() => downloadCSV(sorted, columns)}>
            Export CSV
          </button>
        </div>
      </div>
          ...
          <thead className="bg-gray-50 sticky top-0 z-10">
            <tr>
              {[
                ...
                { key: "team", label: "Team", type: "text" },
                { key: "opp", label: "Opp", type: "text" },
                { key: "sal", label: site === "dk" ? (slot === "flex" ? "DK Sal" : "DK CPT Sal") : (slot === "flex" ? "FD Sal" : "FD MVP Sal"), type: "money" },
                { key: "proj", label: site === "dk" ? (slot === "flex" ? "DK Proj" : "DK CPT Proj") : (slot === "flex" ? "FD Proj" : "FD MVP Proj"), type: "num1" },
                { key: "val", label: site === "dk" ? "DK Val" : "FD Val", type: "num1" },
                { key: "pOwn", label: site === "dk" ? (slot === "flex" ? "DK Flex pOWN%" : "DK CPT pOWN%") : (slot === "flex" ? "FD Flex pOWN%" : "FD MVP pOWN%"), type: "pct" },
                { key: "opt", label: site === "dk" ? (slot === "flex" ? "DK Flex Opt%" : "DK CPT Opt%") : (slot === "flex" ? "FD Flex Opt%" : "FD MVP Opt%"), type: "pct" }
              ].map((c) => (
                <th key={c.key} className={`${header} whitespace-nowrap cursor-pointer select-none`} onClick={() => onSort(c)} title="Click to sort">
                  <div className="inline-flex items-center gap-1">
                    <span>{c.label}</span>
                    {sort.key === c.key ? <span className="text-gray-400">{sort.dir === "desc" ? "▼" : "▲"}</span> : <span className="text-gray-300">▲</span>}
                  </div>
                </th>
              ))}
            </tr>
